...
class DatasetAnalysis():
    ...
    def convert_date(self):
        self.rawData['date'] = pd.to_datetime(self.rawData['date'], format='%Y%m%d')
    ...
    def gender_distribution(self):
        ...
        output_table1 = tabulate(table.items(), headers=['Gender', 'Percentage(%)'], tablefmt='html')
    ...
    def Gender_distribution_language(self):
        ...
        french_speaker = self.frame.query("language =='french'")
        english_speaker = self.frame.query("language =='english'")
    ...
    def Visual_Gender_Bar(self):
        plt.clf()
        language_gender = self.frame.groupby(['language', 'gender'])
        ...
        gender_language = sns.barplot(
            x='gender', y='total', hue='language', data=agg_gender_subset)
    ...
    def Most_Viewed_topics(self):
        ...
        viewtime_std_by_title = self.frame.groupby('item')['pageviewtime'].mean()
    ...
    def MostViewedTopic_Based_on_roles(self):
        plt.clf()
        provider_patient = self.frame.groupby(['item', 'role'])
        ...
        provider_plot = sns.barplot(
            x='total', y='item', hue='role', data=count_subset)
    ...
    def Popular_Topics_languages(self):
        plt.clf()
        item_language = self.frame.groupby(['item', 'language'])
        ...
        item_language = sns.barplot(
            x='total', y='item', hue='language', data=item_subset)
